...
exports.cexpire = exports.cdelData = exports.cdel = exports.cexists = exports.csetData = exports.cset = exports.cgetData = exports.cget = exports.MetadataField = void 0;
...
function cget(pl, context, index, data, sds, transform) {
    let readCount = 0;
    ...
    for (let sd of sds) {
        ...
        pl.get(key, allwantFields, (err, values, valueConvetor) => {
            ...
            if (readCount >= sds.length) {
                let ndata = !transform ? data : transform(data, metadatas);
                if (!ndata.then) {
...
exports.cget = cget;
async function cgetData(cid, data, sds, transform) {
...
async function cset(pl, context, index, data, sds, transform, dataRefs, expireMS) {
    let metadatas = !transform ? undefined : {};
    for (let sd of sds) {
        let hsd = sd;
        if (!hsd.__handled) {
...
exports.cset = cset;
async function csetData(cid, data, sds, transform, expireMS) {
    let pl = getPipeline(cid);
    if (!pl)
        return;
    let context = { data };
