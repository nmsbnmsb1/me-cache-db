...
class List {
    constructor(cid, key, sds, selector, transform = false, expireMS) {
        this.cid = cid;
    ...
    async sel(page, pageSize, order = 'ASC', fields, forceDB) {
        ...
        if (!forceDB) {
            ...
            if (lcdata) {
                ...
                if (count && datas) {
                    datas = await (0, cdata_1.cgetData)(this.cid, datas, sds, this.transform);
                    if (datas) {
                        return {
                            count,
                            page,
        ...
        if (pageData.count <= 0) {
            ...
            for (let i = 0; i < pageData.datas.length; i++) {
                let dref = {};
                (0, cdata_1.cset)(pl, context, i, pageData.datas[i], sds, this.transform, dref, this.expireMS);
                dataRefs.push(dref);
            }
