...
async function sel(cid, data, sds, selector, transform = false, forceDB, expireMS) {
    let ndata;
    if (!forceDB) {
...
exports.sel = sel;
async function selIn(cid, pkfield, pkvalues, sd, selector, transform = false, forceDB, expireMS) {
    let datas = [];
    ...
    if (handleCache === 'update') {
        ...
        if (!Array.isArray(data)) {
            if (await cache.exists(cache.getKey('data', sd.ns, data[sd.pkfield]))) {
                (0, cdata_1.cset)(cache.pipeline(), context, undefined, data, sds, false, undefined, expireMS);
            }
        }
        else {
            ...
            for (let i = 0; i < data.length; i++) {
                if (await cache.exists(cache.getKey('data', sd.ns, data[i][sd.pkfield]))) {
                    (0, cdata_1.cset)(pl, context, i, data[i], sds, false, undefined, expireMS);
                }
            }
        }
    }
